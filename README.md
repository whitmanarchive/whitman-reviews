...
In all of these repositories, you should set `threads: 1` in `config/public.yml` or `config/private.yml`. Otherwise the json file will not be modified (it will break if modified by a multithreaded process).
There will be a message noting `set threads to 1 in private.yml to modify works_and_items file` if you post with a different value under `threads`.
